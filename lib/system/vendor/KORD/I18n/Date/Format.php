...
use KORD\Helper\Date\FormatInterface;
...
class Format implements FormatInterface
{
     ...
     * @param  mixed  $time
     */
    public function __construct(RepositoryInterface $i18n, $time)
    {
        $this->i18n = $i18n;
    ...
    protected function replaceFormat($match)
    {
        switch ($match[1]) {
            ...
            case 'p':  // The current language equivalent of either AM or PM
                return $this->i18n->translate('date.' . (date('G', $this->timestamp) < 12 ? 'am' : 'pm'));
            ...
            case 'x':  // The date in the current language prefered format. en-US: %m/%d/%Y (12/10/2007)
                return $this->format($this->i18n->translate('date.date.short'));
            case 'X':  // The time in the current language prefered format. en-US: %I:%M%p (02:45PM)
                return $this->format($this->i18n->translate('date.time.short'));
    ...
    {
        $string = $this->i18n->translate('date.' . $path, $form);
        return $string[$index];
    }

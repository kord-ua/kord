...
use KORD\I18n\RepositoryInterface;
...
class Date implements DateInterface
{
    ...
    /**
     * Default timestamp format for formattedTime
     * @var  string
     */
     ...
     */
    public function __construct($date_format_closure, RepositoryInterface $i18n, array $config = [])
    {
        $this->date_format_closure = $date_format_closure;
        $this->i18n = $i18n;
        
        if (isset($config['timestamp_format'])) {
            $this->timestamp_format = $config['timestamp_format'];
        }
    ...
    {
        $timestamp_format = ($timestamp_format == null) ? $this->timestamp_format : $timestamp_format;
    ...
    {
        if (!$from) {
            return $this->i18n->translate('date.never');
        }
        if ($to === null) {
            $to = time();
        }
        return $this->getTimePhrase($to - $from);
    }
    ...
    public function getTimePhrase($delta)
    {
        ...
        return $this->i18n->translate('date.' . $msg . $suffix, $delta, array('{delta}' => $delta));
    }
    ...
    public function format($timestamp = null, $format = null)
    {
        ...
        $closure = $this->date_format_closure;
        $time = $closure($timestamp);

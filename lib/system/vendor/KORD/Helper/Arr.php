...
class Arr implements ArrInterface
{
     ...
     * Construct
     * 
     * @param string $delimiter  default delimiter for path()
     */
    public function __construct($delimiter = '.')
    {
        $this->setDefaultDelimiter($delimiter);
    }

    /**
     * Set default delimiter for path() method
     * 
    ...
    public function isArray($value)
    {
        if (is_array($value)) {
            ...
            return (is_object($value) AND $value instanceof \ArrayAccess);
        }
    }
    ...
    public function extract($array, array $paths, $default = null)
    {
        ...
        foreach ($paths as $path) {
            $this->setPath($found, $path, $this->path($array, $path, $default));
        }
    ...
    public function map($callbacks, $array, $keys = null)
    {
        foreach ($array as $key => $val) {
            if (is_array($val)) {
                $array[$key] = $this->map($callbacks, $array[$key]);
